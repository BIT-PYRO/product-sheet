...
const STAGE_DEPT_MAP = {
  rawMaterial:      { deptTo: 'wax-pieces',   deptFrom: 'die',          toLabel: 'Wax Piece',    fromLabel: 'Die' },
  rawSetting:       { deptTo: 'wax-setting',  deptFrom: 'wax-pieces',   toLabel: 'Wax Setting',  fromLabel: 'Wax Piece' },
  wipLiquidCasting: { deptTo: 'casting',      deptFrom: 'wax-setting',  toLabel: 'Casting',      fromLabel: 'Wax Setting' },
  filing:           { deptTo: 'filing',       deptFrom: 'casting',      toLabel: 'Filling',      fromLabel: 'Casting' },
  packing:          { deptTo: 'pre-polish',   deptFrom: 'filing',       toLabel: 'Pre Polish',   fromLabel: 'Filling' },
  setting:          { deptTo: 'hand-setting', deptFrom: 'pre-polish',   toLabel: 'Hand Setting', fromLabel: 'Pre Polish' },
  finalPolish:      { deptTo: 'polishing',    deptFrom: 'hand-setting', toLabel: 'Final Polish', fromLabel: 'Hand Setting' },
  readyForPlacing:  { deptTo: 'plating',      deptFrom: 'polishing',    toLabel: 'Plating',      fromLabel: 'Final Polish' },
}

const DEPT_PIPELINE_ORDER = [
  'die','wax-pieces','wax-setting','casting','filing','pre-polish','hand-setting','polishing','plating','final-stock',
]
...
export function SuggestedVouchersModal({ open, onOpenChange, suggestedItems = [], onVouchersCreated }) {
  const transitionGroups = useMemo(() => {
    const groups = {}
    for (const item of suggestedItems) {
      for (const stage of item.stages) {
        const deptInfo = STAGE_DEPT_MAP[stage.lsKey]
        if (!deptInfo) continue
        const key = `${deptInfo.deptFrom}__${deptInfo.deptTo}`
        if (!groups[key]) {
          groups[key] = { key, deptFrom: deptInfo.deptFrom, deptTo: deptInfo.deptTo, fromLabel: deptInfo.fromLabel, toLabel: deptInfo.toLabel, skuRows: [] }
        }
        groups[key].skuRows.push({ sku: item.sku, category: item.category || '', metal: item.material || '', neededQty: stage.neededQty })
      }
    }
    return Object.values(groups).sort((a, b) => DEPT_PIPELINE_ORDER.indexOf(a.deptTo) - DEPT_PIPELINE_ORDER.indexOf(b.deptTo))
  }, [suggestedItems])
  ...
  const buildInitialForm = useCallback((group) => ({
    ...
    workType: 'In-House',
    deptFrom: group.deptFrom,
    deptTo: group.deptTo,
    noteByIssuer: `Suggested replenishment: ${group.fromLabel} \u2192 ${group.toLabel}`,
    issuedByName: '',
    issuedByContact: '',
    activeTab: 'stone',
    rows: group.skuRows.map((r, i) => ({ id: i + 1, sku: r.sku, category: r.category, metal: r.metal, issuedQty: String(r.neededQty), unit1: 'Pcs', issuedWeight: '', unit2: '' })),
    ...
    forms.forEach((form, formIdx) => {
      ...
      Promise.all(skus.map(async sku => {
        ...
        setForms(prev => {
          ...
          const firstSku = f.rows[0]?.sku?.toLowerCase()
          const firstProduct = firstSku ? productMap.get(firstSku) : null
          if (firstProduct?.stone_entries?.length) {
            f.stoneRows = firstProduct.stone_entries.map((s, i) => ({ id: i + 1, variety: s.variety || '', color: s.color || '', cut: s.cut || '', shape: s.shape || '', length: s.length || '', width: s.width || '', height: s.height || '', qty: s.qty || '' }))
          }
          next[formIdx] = f
          return next
        })
  ...
  const updateForm = (field, value) => {
    setForms(prev => { const next = [...prev]; next[currentStep] = { ...next[currentStep], [field]: value }; return next })
  }
  const updateRow = (id, field, value) => {
    setForms(prev => { const next = [...prev]; const f = { ...next[currentStep] }; f.rows = f.rows.map(r => r.id === id ? { ...r, [field]: value } : r); next[currentStep] = f; return next })
  }
  const addRow = () => setForms(prev => { const next = [...prev]; const f = { ...next[currentStep] }; f.rows = [...f.rows, { id: Date.now(), sku: '', category: '', metal: '', issuedQty: '', unit1: 'Pcs', issuedWeight: '', unit2: '' }]; next[currentStep] = f; return next })
  const deleteRow = (id) => setForms(prev => { const next = [...prev]; const f = { ...next[currentStep] }; f.rows = f.rows.filter(r => r.id !== id); next[currentStep] = f; return next })
  const updateStoneRow = (id, field, value) => setForms(prev => { const next = [...prev]; const f = { ...next[currentStep] }; f.stoneRows = f.stoneRows.map(r => r.id === id ? { ...r, [field]: value } : r); next[currentStep] = f; return next })
  const addStoneRow = () => setForms(prev => { const next = [...prev]; const f = { ...next[currentStep] }; f.stoneRows = [...f.stoneRows, { id: Date.now(), variety: '', color: '', cut: '', shape: '', length: '', width: '', height: '', qty: '' }]; next[currentStep] = f; return next })
  const deleteStoneRow = (id) => setForms(prev => { const next = [...prev]; const f = { ...next[currentStep] }; f.stoneRows = f.stoneRows.filter(r => r.id !== id); next[currentStep] = f; return next })
  const updateDieRow = (id, field, value) => setForms(prev => { const next = [...prev]; const f = { ...next[currentStep] }; f.dieWeightRows = f.dieWeightRows.map(r => r.id === id ? { ...r, [field]: value } : r); next[currentStep] = f; return next })
  const addDieRow = () => setForms(prev => { const next = [...prev]; const f = { ...next[currentStep] }; f.dieWeightRows = [...f.dieWeightRows, { id: Date.now(), dieNumber: '', quantity: '', weight: '', unit: '' }]; next[currentStep] = f; return next })
  const deleteDieRow = (id) => setForms(prev => { const next = [...prev]; const f = { ...next[currentStep] }; f.dieWeightRows = f.dieWeightRows.filter(r => r.id !== id); next[currentStep] = f; return next })
  ...
  async function handleEnrollPerson(name) {
    ...
    if (!exists) {
      await fetch('/api/workforce', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ full_name: n, phone: '', active: true }) }).catch(() => {})
    }
  ...
  async function handleCreateAll() {
    setIsCreating(true)
    setError('')
    const batchId = `suggested-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`
    let count = 0
    try {
      for (const form of forms) {
        const materialRows = form.rows
          .filter(r => String(r.sku || '').trim() && (parseFloat(r.issuedQty) || 0) > 0)
          .map(r => ({ sku: r.sku, category: r.category, metal: r.metal, issued_qty: r.issuedQty, unit1: r.unit1 || 'Pcs', issued_weight: r.issuedWeight || '', unit2: r.unit2 || '' }))
        ...
        const title = `${form.voucherNo} - ${fromLabel} to ${toLabel} (Suggested)`
        const body = {
          title,
    ...
    <>
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className="max-w-[700px] w-[95vw] max-h-[92vh] overflow-y-auto bg-background text-foreground p-0 gap-0 [&>button]:hidden">
          <DialogTitle className="sr-only">Create Suggested Vouchers</DialogTitle>

          {/* Header */}
          <div className="flex items-center justify-between px-4 pt-3 pb-0">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-full bg-orange-400 flex items-center justify-center text-white font-bold text-xs shrink-0">!</div>
              <span className="text-sm font-bold text-midnight-ink">Create Suggested Vouchers</span>
              {totalSteps > 1 && (
            ...
            <div className="flex items-center justify-center gap-1.5 py-2 px-4">
              {transitionGroups.map((g, i) => (
                <button key={g.key} onClick={() => setCurrentStep(i)}
                  className={`h-2 rounded-full transition-all ${i === currentStep ? 'w-6 bg-orange-400' : i < currentStep ? 'w-2 bg-green-400' : 'w-2 bg-gray-300'}`}
          ...
          {created ? (
            <div className="flex flex-col items-center justify-center py-16 gap-4 px-4">
              ...
              <div className="text-center">
                <p className="text-base font-bold text-midnight-ink">{createdCount} suggested voucher{createdCount !== 1 ? 's' : ''} created!</p>
                <p className="text-sm text-cool-gray mt-1">Find them under <strong>Vouchers &rarr; Suggested</strong>. Approve to start production.</p>
              </div>
            ...
            <div className="flex flex-col items-center justify-center py-16 text-cool-gray">
              ...
              <p className="text-sm">All stages are at or above minimum. No vouchers needed.</p>
            </div>
          ) : (
            <div className="px-4 pb-1 flex flex-col gap-2 mt-2">
                ...
                <div className="flex gap-2">
                  <div className="flex flex-col gap-0.5">
                    ...
                    <Select value={form.voucherType} onValueChange={v => updateForm('voucherType', v)}>
                      <SelectTrigger className="h-8 px-2 py-1 text-sm bg-background border-border focus:ring-0 focus:outline-none min-w-[90px]"><SelectValue /></SelectTrigger>
                      <SelectContent>
                ...
                <div className="grid grid-cols-[1fr_auto_1fr] gap-2 items-end">
                  <div className="flex flex-col gap-0.5">
                    ...
                    <Select value={form.issuedTo} onValueChange={v => updateForm('issuedTo', v)}>
                      <SelectTrigger className="h-8 text-sm bg-background border-border focus:ring-0 focus:outline-none"><SelectValue /></SelectTrigger>
                      <SelectContent>
              ...
              <div className="border border-border rounded-md px-2.5 py-2">
                <div className="grid grid-cols-[1fr_auto_1fr] gap-2 items-end">
                  ...
                  <div className="flex items-center justify-center h-8 px-1">
                    <ArrowRight className="h-4 w-5 text-trust-blue" />
                  </div>
                  <div className="flex flex-col gap-0.5">
              ...
              <div className="rounded-md overflow-hidden border border-border">
                <div className="grid grid-cols-[1fr_1fr_1fr_0.8fr_60px_0.8fr_60px_28px] bg-trust-blue text-white text-[9px] font-bold uppercase tracking-wider">
              ...
              <Tabs value={form.activeTab} onValueChange={v => updateForm('activeTab', v)} className="w-full">
                ...
                <TabsContent value="stone" className="mt-1.5">
                  <div className="rounded-md overflow-hidden border border-border">
                    <div className="grid grid-cols-[1fr_1fr_1fr_1fr_0.6fr_0.6fr_0.6fr_0.6fr_28px] bg-trust-blue text-white text-[9px] font-bold uppercase tracking-wider">
                      ...
                      <div key={row.id} className="grid grid-cols-[1fr_1fr_1fr_1fr_0.6fr_0.6fr_0.6fr_0.6fr_28px] border-t border-border items-center bg-background">
                        {['variety','color','cut','shape','length','width','height','qty'].map(field => (
                          <div key={field} className="px-0.5 py-0.5">
                            <Input className="h-6 text-xs bg-background border-border" placeholder={field[0].toUpperCase()+field.slice(1)} value={row[field]} onChange={e => updateStoneRow(row.id, field, e.target.value)} type={field === 'qty' ? 'number' : 'text'} />
                          </div>
                        ))}
                        <div className="flex items-center justify-center">
                ...
                <TabsContent value="die" className="mt-1.5">
                  <div className="rounded-md overflow-hidden border border-border">
                    <div className="grid grid-cols-[1fr_1fr_1fr_1fr_28px] bg-trust-blue text-white text-[9px] font-bold uppercase tracking-wider">
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <div className="flex gap-2 ml-auto">
                  ...
                  <Button size="sm" onClick={handleCreateAll} disabled={isCreating}
                    className="h-8 px-4 text-sm bg-success hover:bg-success/90 text-white font-bold">
                    {isCreating ? <><Loader2 className="mr-1 h-3.5 w-3.5 animate-spin" />Creating...</> : `Create Job Vouchers (${totalSteps})`}
                  </Button>
                </div>
              </div>
