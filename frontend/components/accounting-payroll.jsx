...
function VerifyPage({status,onAction,month}){
  const gDone=STATUS_ORDER.indexOf(status)>STATUS_ORDER.indexOf('received');
  const [vIds,setVIds]=useState(new Set(gDone?MOCK.map(r=>r.id):[]));
  ...
  const allPending=MOCK.filter(r=>!vIds.has(r.id));
  const allDone=MOCK.filter(r=>vIds.has(r.id));
  ...
  const doConfirm=()=>{
    ...
    else{const s=new Set(vIds);cfm.ids.forEach(id=>s.add(id));setVIds(s);setSel(new Set());}
    setCfm(null);
  };
  const names=cfm?cfm.ids.map(id=>MOCK.find(r=>r.id===id)?.name).filter(Boolean):[];
  return(
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:10}}>
        ...
        <PrintExport rows={MOCK} file="payroll_verify" month={month}/>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(155px,1fr))',gap:12}}>
        <Card label="Employees" value={MOCK.length} icon="&#128101;" note={`${MOCK.length} records this month`}/>
        <Card label="Total Gross" value={fmt(MOCK.reduce((s,r)=>s+r.gross,0))} color={C.blue} icon="&#128202;" note="Before deductions"/>
        <Card label="Deductions" value={fmt(MOCK.reduce((s,r)=>s+r.ded,0))} color={C.red} icon="&#10134;" note="PF + TDS + PT"/>
        <Card label="Net Payout" value={fmt(MOCK.reduce((s,r)=>s+r.net,0))} color={C.green} icon="&#128184;" note="To be disbursed"/>
      </div>
      <FilterRow rows={MOCK} q={q} setQ={setQ} dept={dept} setDept={setDept}/>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:10}}>
      ...
      {gDone?<div style={{padding:'12px 16px',background:C.blueBg,border:'1px solid #bfdbfe',borderRadius:10,fontSize:13,fontWeight:600,color:C.blue}}>\u2713 Payroll verified for {month}.</div>
        :vIds.size===MOCK.length?<button onClick={()=>setCfm({ids:MOCK.map(r=>r.id),adv:true})} style={{padding:'12px 24px',background:C.blue,color:'#fff',border:'none',borderRadius:9,fontSize:14,fontWeight:700,cursor:'pointer',alignSelf:'flex-start'}}>\u2192 Send to Approval</button>
        :<span style={{fontSize:13,color:C.muted,fontWeight:600}}>\u2713 {vIds.size}/{MOCK.length} verified &mdash; verify all to proceed</span>}
    </div>
  );
}

function ApprovePage({status,onAction,month}){
  const canAct=status==='verified';
  const gDone=STATUS_ORDER.indexOf(status)>STATUS_ORDER.indexOf('verified');
  const [aIds,setAIds]=useState(new Set(gDone?MOCK.map(r=>r.id):[]));
  const [sel,setSel]=useState(new Set());
  const [ve,setVe]=useState(null);
  const [cfm,setCfm]=useState(null);
  const [sub,setSub]=useState('pending');
  ...
  const [dept,setDept]=useState('');
  const filt=r=>(r.name.toLowerCase().includes(q.toLowerCase())||r.dept.toLowerCase().includes(q.toLowerCase()))&&(!dept||r.dept===dept);
  const allPending=MOCK.filter(r=>!aIds.has(r.id));
  const allDone=MOCK.filter(r=>aIds.has(r.id));
  const pending=allPending.filter(filt);
  const done=allDone.filter(filt);
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:10}}>
        ...
        <PrintExport rows={MOCK} file="payroll_approve" month={month}/>
      </div>
      <FilterRow rows={MOCK} q={q} setQ={setQ} dept={dept} setDept={setDept}/>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:10}}>
      ...
      {gDone?<div style={{padding:'12px 16px',background:C.blueBg,border:'1px solid #bfdbfe',borderRadius:10,fontSize:13,fontWeight:600,color:C.blue}}>\u2713 Payroll approved for {month}.</div>
        :canAct&&aIds.size===MOCK.length?<button onClick={()=>setCfm({ids:MOCK.map(r=>r.id),adv:true})} style={{padding:'12px 24px',background:C.blue,color:'#fff',border:'none',borderRadius:9,fontSize:14,fontWeight:700,cursor:'pointer',alignSelf:'flex-start'}}>\u2192 Send to Payment</button>
        :canAct?<span style={{fontSize:13,color:C.muted,fontWeight:600}}>\u2713 {aIds.size}/{MOCK.length} approved &mdash; approve all to proceed</span>:null}
    </div>
  );
}

function PaymentPage({status,onAction,month}){
  const canAct=status==='approved';
  const gDone=STATUS_ORDER.indexOf(status)>STATUS_ORDER.indexOf('approved');
  const [pIds,setPIds]=useState(new Set(gDone?MOCK.map(r=>r.id):[]));
  const [sel,setSel]=useState(new Set());
  const [ve,setVe]=useState(null);
  const [cfm,setCfm]=useState(null);
  const [sub,setSub]=useState('pending');
  ...
  const [dept,setDept]=useState('');
  const filt=r=>(r.name.toLowerCase().includes(q.toLowerCase())||r.dept.toLowerCase().includes(q.toLowerCase()))&&(!dept||r.dept===dept);
  const allPending=MOCK.filter(r=>!pIds.has(r.id));
  const allDone=MOCK.filter(r=>pIds.has(r.id));
  const pending=allPending.filter(filt);
  const done=allDone.filter(filt);
  ...
  const doConfirm=()=>{
    ...
    else{const s=new Set(pIds);cfm.ids.forEach(id=>s.add(id));setPIds(s);setSel(new Set());}
    setCfm(null);
  };
  const cfmNames=cfm?cfm.ids.map(id=>MOCK.find(r=>r.id===id)?.name).filter(Boolean):[];
  return(
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      <EmpModal emp={ve} onClose={()=>setVe(null)}/>
      {cfm&&<ConfirmModal title={cfm.adv?'Mark Payroll as Paid':'Mark Salary Transferred'} desc={cfm.adv?'All salaries transferred. This will mark payroll as fully paid:':'The following salary transfers will be marked as complete:'} names={cfm.adv?MOCK.map(r=>r.name):cfmNames} nextLabel={cfm.adv?'Mark as Paid':'Mark Transferred'} onConfirm={doConfirm} onCancel={()=>setCfm(null)}/>}
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:10}}>
        ...
        <PrintExport rows={MOCK} file="payroll_payment" month={month}/>
      </div>
      <FilterRow rows={MOCK} q={q} setQ={setQ} dept={dept} setDept={setDept}/>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:10}}>
      ...
      {gDone?<div style={{padding:'12px 16px',background:C.greenBg,border:'1px solid #a7f3d0',borderRadius:10,fontSize:13,fontWeight:600,color:C.green}}>\u2713 Salaries disbursed for {month}.</div>
        :canAct&&pIds.size===MOCK.length?<button onClick={()=>setCfm({ids:[],adv:true})} style={{padding:'12px 24px',background:C.green,color:'#fff',border:'none',borderRadius:9,fontSize:14,fontWeight:700,cursor:'pointer',alignSelf:'flex-start'}}>\u2192 Mark All as Paid</button>
        :canAct?<span style={{fontSize:13,color:C.muted,fontWeight:600}}>{pIds.size}/{MOCK.length} transferred &mdash; transfer all to proceed</span>:null}
    </div>
  );
}

function PostPage({status,onAction,month}){
  const canAct=status==='paid';
  const done=status==='posted';
  const [cfm,setCfm]=useState(false);
  const [sub,setSub]=useState('pending');
  const entries=MOCK.map(r=>[
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      {cfm&&<ConfirmModal title="Post to Accounting" desc="The following journal entries will be posted to the accounting ledger:" names={MOCK.map(r=>r.name)} nextLabel="Post Journal Entries" onConfirm={()=>{onAction('posted');setCfm(false);}} onCancel={()=>setCfm(false)}/>}
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:10}}>
        ...
        <PrintExport rows={MOCK} file="payroll_post" month={month}/>
      </div>
      <SubTabs tab={sub} setTab={setSub} pCount={done?0:MOCK.length} dCount={done?MOCK.length:0} dLabel="Posted"/>
      ...
      {sub==='pending'&&(done?<Empty msg="All entries posted &mdash; see Posted tab"/>:<JournalCards rows={MOCK}/>)}
      {sub==='done'&&(done?<JournalCards rows={MOCK}/>:<Empty msg="No entries posted yet"/>)}
...
export default function AccountingPayroll(){
  const [selMonth,setSelMonth]=useState('Apr');
    ...
    <div style={{fontFamily:'Inter,system-ui,sans-serif'}}>
      ...
      <div className="no-print" style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:20,flexWrap:'wrap',gap:12}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
          <select value={selMonth} onChange={e=>setSelMonth(e.target.value)} style={{padding:'7px 10px',border:'1px solid #e5e7eb',borderRadius:7,fontSize:13,fontWeight:600,color:C.text,background:'#fff',cursor:'pointer',outline:'none'}}>{MONTHS.map(m=><option key={m}>{m}</option>)}</select>
      ...
      {step==='verify'&&<VerifyPage status={status} onAction={setStatus} month={month}/>}
      {step==='approve'&&<ApprovePage status={status} onAction={setStatus} month={month}/>}
      {step==='payment'&&<PaymentPage status={status} onAction={setStatus} month={month}/>}
      {step==='post'&&<PostPage status={status} onAction={setStatus} month={month}/>}
    </div>
  );
}
