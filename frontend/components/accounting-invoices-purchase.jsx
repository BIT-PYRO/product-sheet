      ...
      <div style={{ background: C.surface, borderRadius: 16, padding: 28, width: 480, boxShadow: '0 20px 60px rgba(0,0,0,0.18)', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          <div style={{ gridColumn: '1/-1' }}>
            ...
            <input style={inputStyle} placeholder="e.g. Ravi Suppliers Pvt Ltd" value={form.party_name} onChange={e => set('party_name', e.target.value)} />
          </div>
          <div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 22, justifyContent: 'flex-end' }}>
          ...
          <button onClick={submit} disabled={saving} style={{ padding: '10px 24px', background: saving ? '#9ca3af' : 'linear-gradient(135deg,#dc2626,#b91c1c)', color: '#fff', border: 'none', borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer' }}>
...
function SettleModal({ invoice, accounts, onClose, onSuccess }) {
  ...
  const submit = async () => {
    ...
    } catch (e) { console.error('Bill settle error:', e); setErr('Network error.'); }
    setSaving(false);
  };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: C.surface, borderRadius: 16, padding: 28, width: 400, boxShadow: '0 20px 60px rgba(0,0,0,0.18)' }}>
        <h3 style={{ margin: '0 0 6px', fontSize: 16, fontWeight: 800, color: C.text }}>Pay Bill #{invoice.id}</h3>
        ...
        <p style={{ margin: '0 0 4px', fontSize: 11, color: C.muted, background: C.redBg, padding: '8px 12px', borderRadius: 8, fontWeight: 600 }}>
          ✓ Purchase Expense Dr / Bank Cr — Journal entry will be created automatically.
        </p>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 18, justifyContent: 'flex-end' }}>
          ...
          <button onClick={submit} disabled={saving} style={{ padding: '9px 20px', background: saving ? '#9ca3af' : C.red, color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer' }}>
            {saving ? 'Processing…' : 'Pay & Settle'}
          </button>
        </div>
      </div>
    </div>
...
/* ── Main Component ───────────────────────────────────────────── */
export default function AccountingInvoicesPurchase() {
  ...
  const load = useCallback(async () => {
    ...
    const [inv, acc] = await Promise.all([
      fetch('/api/accounting/invoices?type=purchase').then(r => r.json()).catch(() => null),
  ...
  return (
    <div style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        ...
        <button onClick={() => setShowCreate(true)} style={{ padding: '9px 18px', background: C.red, color: '#fff', border: 'none', borderRadius: 9, fontWeight: 700, fontSize: 13, cursor: 'pointer', boxShadow: '0 2px 8px rgba(220,38,38,0.3)' }}>
          + New Bill
        </button>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: C.slateBg }}>
              {['#', 'Vendor Name', 'Department', 'Amount', 'Due Date', 'Status', 'Action'].map((h, i) => (
                ...
                <td style={{ padding: '12px 14px', textAlign: 'right' }}>
                  {inv.status === 'pending' ? (
                    <button onClick={() => setSettling(inv)}
                      style={{ padding: '6px 14px', background: C.red, color: '#fff', border: 'none', borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                      Pay
                    </button>
                  ) : (
      ...
      {showCreate && <CreateBillModal onClose={() => setShowCreate(false)} onSuccess={load} />}
