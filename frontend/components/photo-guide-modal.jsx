...
export default function PhotoGuideModal({ open, onOpenChange, jobId, voucherNo = "" }) {
  // Each entry: { sku, quantity, unit, images, location: { 'Wax Piece': '...', ... } }
  const [skuData, setSkuData] = useState([])
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState(null)

  // Fetch all SKU+image+location data in one call when the modal opens
  useEffect(() => {
  ...
  // Total qty (numeric sum where possible)
  ...
  // Total colspan = # + Image + SKU + Quantity + 8 location stages
  const totalCols = 4 + LOCATION_STAGES.length
  ...
  function handlePrint() {
    const tableRows = skuData.map((s, i) => `
      <tr>
        ...
        <td class="center qty">${s.quantity || '—'} <span class="unit">${s.unit}</span></td>
        ${LOCATION_STAGES.map(stage => `<td class="loc">${(s.location && s.location[stage]) || ''}</td>`).join('')}
      </tr>
    `).join('')
...
<head>
  <meta charset="utf-8" />
  <title>Photo Guide â€“ ${voucherNo}</title>
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    th { background: #1a56db; color: #fff; font-weight: 600; padding: 4px 5px;
         text-align: left; border: 1px solid #1e40af; white-space: nowrap; }
    th.center { text-align: center; }
    td { padding: 3px 5px; border: 1px solid #d1d5db; vertical-align: middle; }
    td.center { text-align: center; }
    td.sku { font-weight: 700; color: #1e3a8a; }
    td.qty { font-weight: 700; }
    td.loc { color: #374151; font-size: 8px; }
    td.img-cell { padding: 3px; text-align: center; }
    td.img-cell img { width: 55px; height: 55px; object-fit: cover; border-radius: 2px; display: inline-block; margin: 1px; }
    .no-img { width: 60px; height: 60px; background: #f1f5f9; display: inline-flex;
...
<body>
  ...
  <div class="voucher-sub">Voucher: ${voucherNo || 'â€”'} &nbsp;Â·&nbsp; ${skuData.length} SKU(s)</div>
  <table>
    <thead>
      <tr>
        <th class="center" style="width:24px">#</th>
        <th class="center" style="width:68px">Image</th>
        <th>SKU</th>
        <th class="center" style="width:70px">Quantity</th>
        ${LOCATION_STAGES.map(s => `<th>${s}</th>`).join('')}
      </tr>
    </thead>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] w-[95vw] max-h-[90vh] overflow-y-auto bg-background text-foreground p-0 gap-0 [&>button]:hidden">
        ...
        <div className="px-5 py-4 overflow-x-auto">
          {loading ? (
            <div className="flex items-center justify-center py-12 gap-2 text-muted-foreground">
              ...
              <span className="text-sm">Loading photo guideâ€¦</span>
            </div>
          ) : error ? (
              ...
              <table className="w-full border-collapse text-sm">
                <thead>
                  <tr className="bg-[#1a56db] text-white text-xs">
                    ...
                    <th className="border border-[#1e40af] px-3 py-1.5 text-center w-24">Quantity</th>
                    {LOCATION_STAGES.map(stage => (
                      <th key={stage} className="border border-[#1e40af] px-2 py-1.5 text-center whitespace-nowrap">{stage}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {skuData.map((s, i) => (
                    <tr key={s.sku} className={i % 2 === 0 ? 'bg-background' : 'bg-muted/30'}>
                      ...
                      <td className="border border-border px-3 py-1 text-center font-bold text-xs">
                        {s.quantity || '—'} <span className="text-[10px] font-normal text-muted-foreground">{s.unit}</span>
                      </td>
                      {LOCATION_STAGES.map(stage => (
                        <td key={stage} className="border border-border px-2 py-1 text-center text-xs text-gray-700">
                          {(s.location && s.location[stage]) || <span className="text-muted-foreground/40">â€”</span>}
                        </td>
                      ))}
                    </tr>
                  ))}
                ...
                <tfoot>
                  <tr className="bg-blue-50">
                    <td colSpan={3} className="border border-blue-200 px-3 py-2 text-xs font-bold uppercase tracking-wide text-blue-800 text-right">Total Quantity</td>
                    ...
                    {LOCATION_STAGES.map(stage => (
                      <td key={stage} className="border border-blue-200" />
                    ))}
                  </tr>
                </tfoot>
              </table>
