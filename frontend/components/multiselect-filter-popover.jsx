...
export default function MultiselectFilterPopover({
  ...
  const toggleSelection = (option) => {
    ...
    if (isSelected) {
      onSelectValues(selectedValues.filter((v) => v !== option));
    } else {
      onSelectValues([...(selectedValues || []), option]);
    }
  };
        ...
        {mergedOptions.map((option) => {
          ...
          return (
            <button
              ...
              className={`block w-full rounded px-1 py-1.5 text-left text-sm transition ${
                isSelected
                  ? 'bg-blue-200 text-midnight-ink font-medium'
                  : 'text-midnight-ink hover:bg-cloud-gray'
              }`}
            >
              <div className="flex items-center gap-2">
                {isSelected && <Check className="h-4 w-4 text-trust-blue" />}
                <span>{option}</span>
              </div>
            </button>
          );
