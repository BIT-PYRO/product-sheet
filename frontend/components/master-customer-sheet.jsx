...
export default function MasterCustomerSheet() {
	...
	const exportHeaders = CUSTOMER_COLUMNS.filter((col) => col.key !== '__select__').map((col) => col.key);
	const exportLabels = CUSTOMER_COLUMNS.filter((col) => col.key !== '__select__').map((col) => col.label);
	const exportToExcel = () => {
		if (!canExport) return;
		const wb = XLSX.utils.book_new();
		XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([exportLabels, ...sortedCustomers.map((r) => exportHeaders.map((h) => r[h] ?? ''))]), 'Customers');
		XLSX.writeFile(wb, 'customer_data.xlsx');
		setExportMenuOpen(false);
	};
	const exportToPDF = () => {
		if (!canExport) return;
		const rows = sortedCustomers.map((r) => exportHeaders.map((h) => r[h] ?? ''));
		const win = window.open('', '_blank');
		win.document.write(`<html><head><title>Customer Data</title><style>body{font-family:sans-serif;font-size:11px;margin:16px}table{border-collapse:collapse;width:100%}th,td{border:1px solid #ccc;padding:4px 6px;text-align:left}th{background:#dbeafe}</style></head><body><h2>Customer Data</h2><table><thead><tr>${exportLabels.map((l)=>`<th>${l}</th>`).join('')}</tr></thead><tbody>${rows.map((r)=>`<tr>${r.map((c)=>`<td>${c}</td>`).join('')}</tr>`).join('')}</tbody></table><script>window.onload=function(){window.print();}<\/script></body></html>`);
		win.document.close();
		setExportMenuOpen(false);
	};
	const handleExport = () => {
