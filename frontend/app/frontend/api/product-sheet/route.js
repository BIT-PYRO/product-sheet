...
function mapIncomingToProductPayload(data) {
  ...
  return {
    ...
    selling_price: parseFloat(data?.invoicePrice) || 0,
    cost_price: parseFloat(data?.invoicePrice) || 0,
    invoice_price: parseFloat(data?.invoicePrice) || 0,
    is_active: String(data?.shopifyStatus || '').toLowerCase() !== 'inactive',
    die_numbers,
    findings,
    material: String(data?.dropdown1 || data?.material || '').trim(),
    weight: String(data?.weight || data?.weightValue || '').trim(),
    weight_unit: String(data?.weightUnit || '').trim() || 'cts',
    collection: String(data?.dropdown3 || data?.collection || '').trim(),
