...
export default function ProductInventoryPage() {
  ...
  useEffect(() => {
    try {
      ...
      // Keep page usable even if storage data is malformed.
    }
  }, []);
  ...
  const deleteRow = (id) => {
    setRows((prev) => {
      ...
      if (next.length === 0) return [createRow(1)];
    ...
    <main className="min-h-screen bg-cloud-gray">
      ...
      <div className="w-full px-4 md:px-6 pt-20 pb-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-base text-cool-gray">Manage final SKU, value, and unit for products</p>
          </div>
          <Link
            href="/inventory"
            ...
            <table className="w-full min-w-[760px] border-collapse text-sm">
              <thead>
                <tr className="bg-cloud-gray border-b border-soft-border">
                  ...
                  <th className="px-3 py-2 text-left text-xs font-bold uppercase tracking-wide text-cool-gray">Final SKU</th>
                  <th className="px-3 py-2 text-left text-xs font-bold uppercase tracking-wide text-cool-gray">Value</th>
